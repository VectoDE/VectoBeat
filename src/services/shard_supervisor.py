...
class ShardSupervisor:
    ...
    async def _restart_shard(self, shard_id: int, *, reason: str) -> None:
        ...
        ws = getattr(shard, "ws", None) if shard else None
        if not ws:
            launcher = getattr(self.bot, "launch_shard", None)
            if launcher:
                self.logger.info("Shard %s missing websocket; relaunching (%s).", shard_id, reason)
                try:
                    await launcher(shard_id=shard_id)
                except TypeError:
                    await launcher(shard_id)
                except Exception as exc:  # pragma: no cover - defensive
                    self.logger.error("Failed to relaunch shard %s: %s", shard_id, exc)
            else:
                self.logger.warning("Shard %s missing websocket and no launcher available (%s).", shard_id, reason)
            return
