...
export const sanitizeDomain = (value?: string | null) => {
...
export const sanitizeEmail = (value?: string | null) => {
  if (typeof value !== "string") return ""
...
export const createDomainBrandingHandlers = (deps: RouteDeps = {}) => {
  ...
  const POST = async (request: NextRequest) => {
    ...
    const current = (await fetchSettings(guildId)) as ServerFeatureSettings
    ...
    const merged = await saveSettings(guildId, discordId, {
      ...current,
      ...updates,
    })
...
export const { POST } = createDomainBrandingHandlers()
