...
export default function GlobalError({ error, reset }: GlobalErrorProps) {
  ...
  useEffect(() => {
    console.error("[VectoBeat] Global UI failure", {
      correlationId,
      name: error?.name,
    ...
    <html>
      <body className="min-h-screen bg-background text-foreground">
        <div className="flex min-h-screen items-center justify-center px-4 py-12">
          <div className="w-full max-w-xl rounded-2xl border border-border/50 bg-card/80 px-6 py-8 shadow-2xl backdrop-blur">
            <div className="space-y-1 text-center">
              ...
              <p className="text-sm text-foreground/70">
                Something went wrong while rendering the control panel. The incident below was logged so we can trace it.
              </p>
            </div>
