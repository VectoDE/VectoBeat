...
- Run `python3 scripts/validate_env.py` and `npm run test:server-settings` before tagging.
- Apply database migrations before cutting traffic over: `cd frontend && npx prisma migrate deploy`.

## Restarts & scaling
- Frontend and bot are safe to restart independently; queue snapshots are persisted in the durable queue-sync store.
...
- Toggle chaos/scaling experiments via `.env` (`CHAOS_ENABLED`, `SCALING_ENABLED`) and tail `bot/logs` for outcomes.
...
- Queue sync durability: check `redis` keys/TTL (or your MySQL durable store) and confirm `/api/dashboard/analytics` matches recent socket events.
